...
namespace simpletest
{
     ...
     *  \ref llama::DatumStruct, which can be shortend to llama::DS. This takes
     * a template list of all members of this struct-like domain. Every member
     * needs to be a \ref llama::DatumElement, which can itself be shortend to
     * llama::DE. A DatumElement is a list of two elements itself, first the
     * name of the element as type and secondly the element type itself, which
     * may be a nested DatumStruct. A handy shortcut is \ref llama::DatumArray
     * (llama::DA), which defines multuple datum elements with an anonymous name
    ...
    /** Example functor for \ref llama::ForEach which can also be used to print
     * the coordinates inside of a datum domain when called.
     */
    ...
    int main(int argc, char ** argv)
    {
        ...
        std::cout << "AoS address of (0,100) <0,1>: "
                  << llama::mapping::AoS<UD, Name>(udSize).getBlobByte<0, 1>(
                         {0, 100})
                  << std::endl;
        std::cout << "SoA address of (0,100) <0,1>: "
                  << llama::mapping::SoA<UD, Name>(udSize).getBlobByte<0, 1>(
                         {0, 100})
                  << std::endl;
        ...
        // Defining the factory type based on the mapping and the chosen
        // allocator
        using Factory
        ...
        // using the position in the user domain and a tree coord or a uid in
        // the datum domain to get the reference to an element in the view
        ...
        // will change based on the chosen mapping. When array of struct is
        // chosen instead the elements will be much closer than with struct of
        // array.
        std::cout << &position_x << std::endl;
        std::cout << &momentum_z << " "
                  << (size_t)&momentum_z - (size_t)&position_x << std::endl;
