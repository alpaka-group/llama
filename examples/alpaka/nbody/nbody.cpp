...
pPInteraction(T_VirtualDatum1 && p1, T_VirtualDatum2 && p2, const Element & ts)
{
            ...
            ++b)
        {
            ...
            for(auto pos = start; pos < end; ++pos)
                if constexpr(NBODY_USE_SHARED)
                    ...
                    pPInteraction(particles(pos), particles(start2 + pos2), ts);
            if constexpr(NBODY_USE_SHARED)
                alpaka::block::sync::syncBlockThreads(acc);
        }
    }
...
int main(int argc, char ** argv)
{
    ...
    for(std::size_t s = 0; s < steps; ++s)
    {
        ...
        alpaka::kernel::exec<Acc>(queue, workdiv, updateKernel, mirrorView, ts);

        chrono.printAndReset("Update kernel");

        MoveKernel<problemSize, elemCount> moveKernel;
        alpaka::kernel::exec<Acc>(queue, workdiv, moveKernel, mirrorView, ts);
