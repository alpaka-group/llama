    ...
    LLAMA_FN_HOST_ACC_INLINE void operator()(const Acc& acc, View oldImage, View newImage) const
    {
        ...
        if constexpr (SHARED)
        {
            ...
            const std::size_t bEnd[2] = {
                alpaka::math::min(acc, bStart[0] + ElemsPerBlock + 2 * KernelSize, oldImage.mapping.userDomainSize[0]),
                alpaka::math::min(acc, bStart[1] + ElemsPerBlock + 2 * KernelSize, oldImage.mapping.userDomainSize[1]),
            };
        ...
        const std::size_t end[2] = {
            alpaka::math::min(acc, start[0] + Elems, oldImage.mapping.userDomainSize[0] - 2 * KernelSize),
            alpaka::math::min(acc, start[1] + Elems, oldImage.mapping.userDomainSize[1] - 2 * KernelSize),
        };
