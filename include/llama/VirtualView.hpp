...
/** Struct which acts like a \ref View, but shows only a smaller and/or shifted
 *  part of a parental (virtual) view. A virtual view does not hold any memory
 *  itself but has a reference to the parent view (and its memory).
 * \tparam T_ParentViewType type of the parent, can also be another VirtualView
 */
template<
...
struct VirtualView
{
     ...
     * \param parentView a reference to the parental view. Meaning, the parental
     *  view should not get out of scope before the virtual view.
     ...
     *  (\ref VirtualDatum). Should be favoured to access data because of the
     *  more array of struct like interface and the handy intermediate
     *  \ref VirtualDatum object.
     * \param userDomain user domain as \ref UserDomain
     * \return \ref VirtualDatum with bound user domain, which can be used to
