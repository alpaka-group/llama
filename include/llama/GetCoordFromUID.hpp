...
/** Converts a coordinate in a datum domain given as UID to a \ref DatumCoord
 *  relative to a given datum coord in the tree. The returned datum coord is
 *  also relative to the input datum coord, that is the sub tree.
 * \tparam T_DatumDomain the datum domain (\ref DatumStruct)
 * \tparam T_DatumCoord datum coord to start the translation from UID to datum
 *  coord
 * \tparam T_UID... the uid of in the datum domain, may also be empty (for
 *  `DatumCoord< >`)
 * \returns a \ref DatumCoord with the datum domain tree coordinates as template
...
template<
    ...
    typename... T_UID
>
using GetCoordFromUIDRelative = typename internal::GetCoordFromUIDRelativeImpl<
    T_DatumDomain,
    T_DatumCoord,
    T_UID...
>::type;
